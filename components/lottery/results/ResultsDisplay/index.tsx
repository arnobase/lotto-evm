...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ results, isLoading }) => {
  ...
  return (
    <div className="space-y-6">
      {results.map((result) => (
        <div key={result.drawNumber} className="bg-gray-800 rounded-lg p-6">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            {result.numbers.map((number, index) => (
              <div
                key={index}
                className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white"
              >
                {number}
              </div>
            ))}
