...
export const useLastParticipations = () => {
  ...
  // Traiter les participations de l'indexeur avec useMemo
  const processedParticipations = useMemo(() => {
    const processedIndexerParticipations = indexerParticipations.map(p => ({
    ...
    // Combiner et trier toutes les participations
    return [
      ...localParticipations,
      ...processedIndexerParticipations
    ]
    ...
    .slice(0, MAX_HISTORY);
  }, [indexerParticipations, localParticipations]);
